...
- **模式选择与如何使用**
  ...
  **5. 注意事项**
  - **真实SKU / 虚拟SKU 一行只填其中一个。** 两边都填时：拆表、分组按**虚拟SKU**认这一行；去 SKU 表配素材时真实、虚拟都会查，用**条数更多**的那份（一样多则用虚拟）。结果表两列仍按账号表原样带出。中台导品一般也是二选一，建议导出后确认不要两列都有值。
  - SKU 表一行若真实、虚拟都填了，这条素材会同时挂到两个码下，填错列也能对上；规范仍是只填一列。
