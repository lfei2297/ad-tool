...
def run(params):
    ...
    sig = (up.name if up else None, int(target_total_rows))
    if st.session_state.get("m6_sig") != sig:
        st.session_state["m6_sig"] = sig
        st.session_state.pop("m6_xlsx", None)
        st.session_state.pop("m6_msg", None)
        st.session_state.pop("m6_filename", None)
    ...
    if up and st.button("🚀 开始组合循环构建", key="m6_btn"):
        ...
        if final_rows:
            ...
            st.session_state["m6_xlsx"] = write_excel_final(final_df, "组合填充结果", local_params)
            st.session_state["m6_msg"] = (
                f"✅ 模块六处理完成！成功识别出 {len(groups)} 个产品组合并填充至 {target_total_rows} 行！"
            )
            st.session_state["m6_filename"] = f"{file_prefix}组合填充_{target_total_rows}行.xlsx"
            del df_raw, df_clean, final_rows, final_df, groups
            gc.collect()

    if "m6_xlsx" in st.session_state:
        st.success(st.session_state.get("m6_msg", "生成完毕！"))
        st.download_button(
            f"💾 下载：{target_total_rows}行组合总表",
            data=st.session_state["m6_xlsx"],
            file_name=st.session_state.get("m6_filename", "组合填充.xlsx"),
            key="dl_m6",
        )