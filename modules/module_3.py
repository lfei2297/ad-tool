...
def run(params):
    ...
    if f2:
        if st.button("🚀 开始智能分组", key="m3_btn_s2"):
            ...
            # ✨✨✨【核心修正点】✨✨✨
            # 在进入 smart_logic 前，如果在步骤二上传的是带原始素材数量的表
            # 我们在这里显式地、强行进行过程展开，把最新的 params 怼进去
            if "广告素材数量" in df_in.columns:
                st.caption(f"⚙️ 检测到上传为原始表，正在强制执行【{params['repeat_1']} 次】素材克隆展开...")
                df_in = process_step1(df_in, params)
            
            # 此时传入 smart_logic 的 df_in，已经是被百分之百复制膨胀过的干净明细了
            res_df = smart_logic(df_in, group_size, params)
            
            if not res_df.empty:
                st.success(f"✅ 步骤二智能分组成功！（已确保每个素材独立散落重复）")
...
def process_step1(df, params):
    """专门负责将原始表按素材数量展开的逻辑"""
    expanded_rows = []
    curr_repeat = int(params.get('repeat_1', 1))
...
def smart_logic(df, size, params):
    """智能分组核心避让算法（接收已经展开就绪的明细表）"""
    # 数据清洗：剔除无关行
    df_clean = df[~df["真实SKU"].astype(str).str.contains("总计|空白", na=False)].copy()
    
    # 智能强配对：把空字符串、空格、"空白"全部清洗为 None，确保能100%抓取到虚拟SKU作为备用代号
