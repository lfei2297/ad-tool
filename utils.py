...
def write_excel_final(df, sheet_name, params, is_m3=False, color_by=None):
    ...
    target_order = [
        ...
        "国家", "着陆页版本名称", "广告素材版本名称", "广告素材ID", "出价/竞价", "系列标注", "注意事项"
    ]
    ...
    if hint_df is not None and not hint_df.empty:
        ...
        hints = {
            ...
            "广告素材版本名称": "产品库中的具体广告素材版本名称，会取该版本下最新的一条广告素材；广告素材版本名称和广告素材ID这两列二选一",
            "广告素材ID": "产品素材库中具体的广告素材ID；广告素材版本名称和广告素材ID这两列二选一",
...
def build_template(columns_dict, sheet_name):
    ...
    with pd.ExcelWriter(out, engine="xlsxwriter") as writer:
        ...
        for i, (col_name, hint_text) in enumerate(columns_dict.items()):
            ...
            elif col_name in ["注意事项", "着陆页版本名称", "广告素材版本名称", "广告素材ID"]:
...
def get_template_m8():
    """
    构建模块八专用模板（字段更名为 广告素材版本名称，包含广告素材ID，无锁定）
    """
    acc_cols = {
        "广告账号ID": "",
        "主页ID": "可不填，不填则使用资产管理中的默认主页",
        ...
        "出价/竞价": "如需指定“真实/虚拟SKU”与“出价/竞价”的关系，请填写，最多2位小数，可不填，不填则全不填，填了则全填",
        "系列标注": "可不填",
        "SKU组": "模式四/五：同一账号下相同数字的 SKU 打进同一系列，建议填 1、2、3；留空则该账号全部 SKU 为一组。模式一/二/三可不填",
        "Unnamed: 12": "此行为说明，勿删除，请从第三行开始填写"
    }
