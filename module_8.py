...
- **模式选择与如何使用**
  ...
  **2. 配置参数与提取模式**（先选场景，再选提取方式）
  ...
  - **限制单个SKU上限**：仅多品、勾选后才出现（默认 20）。每个 SKU 只取最新的前若干条。不勾选则不限制（模式五才是该 SKU 全量）。
  ...
  **多品多素材**
  ...
  - **模式五 · 全量提取**：同一 `SKU组` 的素材全部放在**同一张表**。默认不截单个 SKU；若勾选「限制单个SKU上限」，每个 SKU 只取最新 N 条后再全放进这一张表。同一账号超过 n 条时**不拆文件**；导品设广告数 = n 并勾选继续创建，中台会在同账号下开下一系列。
