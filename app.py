...
# 页面配置
# ==========================
st.set_page_config(page_title="广告素材生成工具 v11", page_icon="🚀", layout="wide")
st.title("🚀 广告素材批量生成工具 (V11 全功能旗舰版)")
...
FAST_MODE = st.sidebar.checkbox("开启极速模式 (跳过渲染以提升速度)", value=False)
...
def smart_grouping_logic(df_to_group, group_size=30):
    ...
    for idx, bucket in enumerate(buckets):
        for row in bucket:
            row["分组"] = f"分组{idx+1}"
            row["备注"] = f"分组{idx+1}满足{len(bucket)}个"
...
if "模块三" in PROCESS_MODE:
    ...
    if f1:
        ...
        for _, row in raw.iterrows():
            for v in expand_material_versions(row):
                ...
                for _ in range(REPEAT_FIRST): exp_rows.append(nr.copy())
        st1_df = pd.DataFrame(exp_rows)
        ...
        if "广告素材数量" in df_in.columns:
            ...
            for _, row in df_in.iterrows():
                for v in expand_material_versions(row):
                    nr = row.copy(); nr["广告素材版本名称"] = v
                    for _ in range(REPEAT_FIRST): exp_rows.append(nr.copy())
            df_for_g = pd.DataFrame(exp_rows)
            if REPEAT_SECOND > 1: df_for_g = pd.concat([df_for_g] * REPEAT_SECOND, ignore_index=True)
        else:
            df_for_g = df_in
        res = smart_grouping_logic(df_for_g)
        st.download_button("💾 导出智能分组的总表", data=write_excel_clean(res, "分组结果", True), file_name=f"{FILE_PREFIX}智能分组总表.xlsx")

else:
    up = st.file_uploader("📂 上传素材表格 (.xlsx)", type=["xlsx"])
    if up and st.button("🚀 开始处理"):
        ...
        for _, row in df_raw.iterrows():
            ...
            tmp_df = pd.DataFrame(row_rows)
            if REPEAT_SECOND > 1: tmp_df = pd.concat([tmp_df] * REPEAT_SECOND, ignore_index=True)
            all_expanded.append(tmp_df)
            
            if "模块一" in PROCESS_MODE:
                m1_groups[f"素材数_{m_len}"].append(tmp_df)
            elif "模块二" in PROCESS_MODE:
                sku = str(row.get("真实SKU","")).strip() or str(row.get("虚拟SKU","")).strip()
                country = str(row.get("国家","")).strip()
                m2_agg[(sku, country)].append(tmp_df)
                m2_counts[(sku, country)] += m_len

        total_df = pd.concat(all_expanded, ignore_index=True)
        tasks["总表"] = total_df
        
        if "模块一" in PROCESS_MODE:
            for k, v in m1_groups.items(): tasks[k] = pd.concat(v, ignore_index=True)
        elif "模块二" in PROCESS_MODE:
            # 聚合拆分：将同一 SKU+国家的 放到一个文件里，文件名体现总素材数
            final_m2 = defaultdict(list)
            for (sku, country), dfs in m2_agg.items():
                total_m = m2_counts[(sku, country)]
                c_tag = f"{country}_" if country else ""
                final_m2[f"{c_tag}素材数_{total_m}"].append(pd.concat(dfs, ignore_index=True))
            for k, v in final_m2.items(): tasks[k] = pd.concat(v, ignore_index=True)

        zip_b = io.BytesIO()
        with zipfile.ZipFile(zip_b, "w") as zf:
            for name, d in tasks.items():
                zf.writestr(f"{FILE_PREFIX}{name}.xlsx", write_excel_clean(d, "Data"))
        st.download_button("📦 下载处理结果包", data=zip_b.getvalue(), file_name=f"{FILE_PREFIX}处理结果.zip")
